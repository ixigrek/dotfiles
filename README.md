...
profil AWS / son contexte Kubernetes devient actif ; on en sort, plus aucun
provider n'est actif — ni pour les CLI, ni dans le prompt.
...
use kube staging equipe-x   # → KUBECONFIG (namespace optionnel)
```
...
| `.config/direnv/direnvrc` | fournit `use gcloud`, `use azure`, `use aws`, `use kube` |
